...
pub async fn checkout_branch(
    ...
    run_git_mutation(
        ...
        move |path, git| checkout(&path, &git, &name).map_err(|e| e.to_string()),
    )
    .await
}
...
pub async fn create_new_branch(
    ...
    run_git_mutation(
        state.git_service.clone(),
        optional_repo_path(repo_path),
        move |path, git| create_branch(&path, &git, &name).map_err(|e| e.to_string()),
    )
    .await
}
...
pub async fn update_local_branch(
    ...
    run_git_mutation(
        state.git_service.clone(),
        optional_repo_path(repo_path),
        move |path, git| update_branch(&path, &git, &branch).map_err(|e| e.to_string()),
    )
    .await
}
...
pub async fn delete_existing_branch(
    ...
    run_git_mutation(
        state.git_service.clone(),
        optional_repo_path(repo_path),
        move |path, git| delete_branch(&path, &git, &name, force).map_err(|e| e.to_string()),
    )
    .await
}
