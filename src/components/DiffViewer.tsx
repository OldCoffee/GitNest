...
export function DiffViewer({ tab }: DiffViewerProps) {
  ...
  const previewQuery = useQuery({
    queryKey: ["preview", tab.id, tab.path, tab.mode, tab.commitHash],
    queryFn: () =>
      api.getFilePreview(tab.path, previewMode, tab.commitHash ?? null),
  ...
  const branchDiffQuery = useQuery({
    ...
    queryFn: async () => {
      if (tab.mode === "branch" && tab.baseBranch && tab.headBranch) {
        const diff = await api.getDiffBranchRange(tab.baseBranch, tab.headBranch, tab.path);
        return fileDiffToPreview(diff);
      }
      if (tab.mode === "branch_working" && tab.headBranch) {
        const diff = await api.getDiffBranchWorking(tab.headBranch, tab.path);
        return fileDiffToPreview(diff);
      }
