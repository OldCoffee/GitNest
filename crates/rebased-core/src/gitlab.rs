...
pub fn create_merge_request(
    account: &GitLabAccount,
    project: &str,
    options: &crate::CreateMergeRequestOptions,
) -> Result<MergeRequestEntry> {
    let encoded = urlencoding::encode(project);
    let host = account.host.trim_end_matches('/');
    let url = format!("{host}/api/v4/projects/{encoded}/merge_requests");
    ...
    let item: serde_json::Value = response
        .into_json()
        .map_err(|e| crate::RebasedError::other(e.to_string()))?;
    Ok(MergeRequestEntry {
        iid: item
            .get("iid")
            .and_then(|v| v.as_u64())
            .ok_or_else(|| crate::RebasedError::other("create MR: missing iid"))?,
        title: item
            .get("title")
            .and_then(|v| v.as_str())
        ...
        url: item
            .get("web_url")
            .and_then(|v| v.as_str())
            .ok_or_else(|| crate::RebasedError::other("create MR: missing web_url"))?
            .to_string(),
